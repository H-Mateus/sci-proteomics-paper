...
Further, AIS grade A ($n = 20$) and D ($n = 21$) at <2 weeks ("Acute", $n = 39$) and approx. 
...
Further label-free quantitation proteomics was performed to also assess lower abundance proteins, in which proteins were equilibrated using ProteoMiner&trade; beads. 
...
#### example of a plot grid

There's good documentation for using `cowplot` that can be found [here](https://wilkelab.org/cowplot/articles/plot_grid.html)
Here's an example using the elisa plots

(ref:plot-grid-example) Example of arranging plots in a grid.

```{r, label='plot-grid-example', fig.width=7, fig.height=7, fig.cap='(ref:elisa-sig-plots4)'}
plot_grid(plotlist = elisa_sig_plots,
          labels = "AUTO", ncol = 2, axis = "brtl", align = "hv")
```

Alternative with `patchwork`, docs [here](https://patchwork.data-imaginist.com/index.html)
Could be improved by cleaning up the x axis, either reduce font size or rotate labels 45 or 90 degrees

```{r, label='patchwork-example'}
## add plots together
...
patchwork_plots[[3]] = patchwork_plots[[3]] + theme(axis.title.x = element_blank())
...
```{r, label='patchwork-2', fig.height = 10}
patchwork_plots <- Reduce("/",elisa_sig_plots)
