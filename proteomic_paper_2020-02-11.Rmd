...
pkg_bioconductor <- c("ReactomePA", "STRINGdb")
...
Globally, SCI affects around 500,000 people each year and is most commonly the result of road traffic accidents or falls.[@crozier-shaw_management_2020]
...
The lifetime cost of care in the UK is estimated to be £1.12 million (mean value) per SCI, with the total cost of SCI in 2016 in the UK being £1.43 billion.[@mcdaid_understanding_2019]
...
The systemic inflammatory response that follows SCI can frequently result in organ complications, particularly in the liver and kidneys.[@gris_systemic_2008; @sun_multiple_2016]

The underlying pathological mechanism of SCI remain unclear, recovery of neurological function is highly variable, requiring any clinical trials to have an impractically large sample size to prove efficacy making development of novel therapies extremely difficult.[@spiess_conversion_2009]
...
Many studies investigating novel biomarkers for SCI use cerebral spinal fluid (CSF) as the closer proximity is thought to be more reflective of the parenchymal injury.[@kwon_neurochemical_2019; @hulme_developing_2017]
...
Studies of traumatic brain injury have also demonstrated that protein markers identified in CSF are also detectable in both plasma and serum.[@wang_update_2018]
Similarly, inflammatory markers in acute SCI patients were found to be mirrored in blood and CSF, albeit at a lower concentration.[@kwon_cerebrospinal_2010]

Various proteins have been shown to be altered in the blood of persons with SCI, including multiple interleukins (IL), tumour necrosis factor alpha and C-reactive protein (CRP).[@segal_circulating_1997; @hayes_elevated_2002; @frost_inflammatory_2005]
Previously we found routinely collected blood measures associated with liver function and inflammation added predictive value to AIS motor and sensor outcomes at discharge and 12-months post-injury.[@bernardo_harrington_routinely_2020; @brown_preliminary_2019]
...
Data analyses were performed with the statistical programming language `r R.version$version.string`, please see table \@ref(tab:package-table) for a list of packages used and the respective version numbers.[@base; @bibtex; @BiocManager; @bookdown1; @captioner; @caret; @data.table; @datasets; @DiagrammeR; @dplyr; @forcats; @Formula; @ggplot2; @ggVennDiagram; @glmnet1; @graphics; @grDevices; @Hmisc; @huxtable; @kableExtra; @knitr3; @lattice; @lime; @lubridate; @Matrix; @methods; @mice; @MSstats; @naniar; @psych; @purrr; @RColorBrewer; @ReactomePA; @readr; @readxl; @remotes; @rlang; @rmarkdown1; @rmarkdown2; @rmarkdown3; @stats; @STRINGdb; @stringr; @survival1; @survival2; @thesisdown; @tibble; @tidyr; @tidyverse; @utils; @zoo]
...
Within minutes CNS cells at the lesion site have been found to secrete several pro-inflammatory mediators, including TNF-$\alpha$ and interleukins, in both rodent models and human patients with SCI.[@pineau_proinflammatory_2007; @chandrasekar_acute_2017; @dalgard_cytokine_2012; @bastien_il-1_2015]
