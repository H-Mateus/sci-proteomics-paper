---
title: "Proteomic paper"
output:
  bookdown::word_document2:
    #  - --reference-doc=extras/Reference_Document.docx # this can cause file 'NA' error
    pandoc_args:
  ...
  html_document:
author:
  - Gabriel Mateus Bernardo Harrington, BSc:
...
keywords: "Spinal cord injury, biomarker, prognostic modelling, blood, outcome"
link-citations: yes
number_sections: yes
csl: extras/journal-of-neurotrauma.csl
toc: false
geometry: margin=2cm
always_allow_html: true
date: "`r format(Sys.time())`"
# subset bib file in progress
bibliography: extras/paper_and_pkgs.bib
#bibliography: extras/masterLib.bib
---
...
pkg <- c("kableExtra", "tidyverse", "captioner", "bibtex",
         ...
         "knitr", "BiocManager", "purrr", "base", "readxl",
         "lubridate", "DiagrammeR", "ggVennDiagram", "RColorBrewer")
...
```{bash, label='glossery-gen-command', include=FALSE, eval=FALSE}
## Use the following to generate the glossary
## make sure you're in the paper directory

## generate list of abbreviations - note needs to be done after compiling the thesis once
makeindex proteomic_paper_2020-02-11.nlo -s nomencl.ist -o proteomic_paper_2020-02-11.nls
## then recompile paper to update abbrev list
```
...
Furthermore, patients with SCI show markedly higher rates of mental illness relative to the general population.[@furlan_health_2017]
...
The underlying pathological mechanism of SCI remains unclear; recovery of neurological function is highly variable, requiring any clinical trials to have an impractically large sample size to prove efficacy, hence the translation of novel efficacious therapies is challenging and expensive.[@spiess_conversion_2009]
...
Many studies investigating novel biomarkers for SCI use cerebral spinal fluid (CSF) as the closer proximity of this medium is thought to be more reflective of the parenchymal injury.[@kwon_neurochemical_2019; @hulme_developing_2017]
...
Further, inflammatory marker levels detected in acute SCI patients were found to be mirrored in donor-matched blood and CSF, albeit at lower absolute concentrations systemically.[@kwon_cerebrospinal_2010]

A number of individual proteins have been shown to be altered in the bloods post-SCI, including multiple interleukins (IL), tumour necrosis factor alpha (TNF-$\alpha$) and C-reactive protein (CRP).[@segal_circulating_1997; @hayes_elevated_2002; @frost_inflammatory_2005] Glial cells (astrocytes, oligodendrocytes and microglia) have also been shown to be activated post-injury, forming glial scars that worsen inflammation at the lesion site. 

- Reference -

Previously, we have shown that routinely collected blood measures associated with liver function and inflammation added predictive value to AIS motor and sensor outcomes at discharge and 12-months post-injury.[@bernardo_harrington_routinely_2020; @brown_preliminary_2019] The current study uses an unbiased shotgun proteomic approach to investigate differentially expressed proteins in SCI patients, coupled with bioinformatics analyses. 
...
### Proteomics cohort {#itraq-patient-demo}
...
Plasma samples were taken from `r nrow(prot_demo)` patients (Table \@ref(tab:patient-demo-chap3)).
...
#### Protein assay and plasma pooling

$2\mu l$ of thawed plasma were diluted with $98\mu l$distilled water.
Total protein was quantified using a Pierce&trade; $660 nm$ Protein Assay (Thermo Fisher Scientific, Hemel Hempstead, UK)[@stoscheck_protein_1987].
In brief, $10\mu l$ of the diluted sample was added to $150\mu l$ of Thermo Pierce 660nm protein assay reagent in triplicate and the optical density was read at 660nm using an FLUOstar&reg; Omega plate reader.
...
(ref:mer-itraq-flow) Flow diagram illustrating part of the iTRAQ sample processing.

```{r, label='mer-itraq-flow', fig.cap='(ref:mer-itraq-flow)', out.width="100%"}
## mermaid flowdiagram. note the custom css colours isn't working
mermaid("
graph TD
    A(Peptide mixture of acute AIS C improvers) --> B(Label all peptides with tag-114)
    B --> C{Mix and separate via<br> Liquid Chromatography}
    C --> D(Identify and quantify proteins via Mass Spectrometry)
    E(Peptide mixture of acute AIS C non-improvers) --> F(Label all peptide with tag-115)
    F --> C
...
iTRAQ-labelled peptides were re-suspended in $0.6 ml$ of loading buffer ($10 mM$ monopotassium phosphate [KH~2~PO~4~], 20% acetonitrile [MeCN], pH 3.0), followed by sonication.
The pH was adjusted to $3.0$ with $0.5 M$ orthophosphoric acid (H~3~PO~4~).
...
A gradient of 0–50% Bscx (10 mM [KH~2~PO~4~], 20% MeCN, $500 mM$ KCl, pH 3.0) was applied for 20 minutes, 50–100% Bscx for 3 minutes, followed by 100% Bscx for another 3 minutes to wash the column, prior to re-equilibration in 100% Ascx for a further 11 minutes.
...
#### Mass spectromety analysis

A total of 12 SCX fractions were analysed by nano-electrospray ionisation-liquid chromatography/tandem mass spectrometry (LC-MS/MS) \nomenclature{LC-MS/MS}{Liquid chromatography/tandem mass spectrometry}using a TripleTOF 5600 tandem mass spectrometer (AB Sciex, Framingham, MA, USA) as described previously.[@fuller_stathmin_2015]
...
#### Plasma preparation and protein equalisation using ProteoMiner&trade;

To reduce the dynamic range of proteins, ProteoMiner&trade; beads (BioRad, Hemel Hempstead, UK) were used.
...
Total protein was quantitated with a Pierce&trade; $660 nm$ Protein Assay (Thermo Fisher Scientific, Hemel Hempstead, UK), whereupon 5 mg of total protein was applied to ProteoMiner&trade; beads.[@stoscheck_protein_1987]
After a washing step, bead-bound proteins 0.05% (w/v) RapiGest (Waters, Manchester, UK) was applied in 25 mM ammonium bicarbonate for 10 min at 80&deg;C precessing reduction, alkylation, and in-situ protein digestion without removing the beads to ensure the complete proteome is preserved.
...
The supernatant fractions were frozen at -80&deg;C before LC-MC/MC.

#### LC separation and Quadrupole-Orbitrap instrument

The tryptic peptide were subjected to LC-MC/MC via a 2-h gradient on a NanoAcquity&trade; ultraperformance LC (Waters, Manchester, UK) connected to a Q-Exactive Quadrupole-Orbitrap instrument (Thermo-Fisher Scientific Hemel Hempstead, UK).
...
### iTRAQ data processing {#itraq-stats}

Data analyses were performed with the statistical programming language `r R.version$version.string`, please see table \@ref(tab:package-table) for a list of packages used and the respective version numbers.[@base; @bibtex; @BiocManager; @bookdown1; @captioner; @caret; @data.table; @datasets; @DiagrammeR; @forcats; @Formula; @ggVennDiagram; @glmnet1; @graphics; @grDevices; @Hmisc; @huxtable; @kableExtra; @knitr3; @lattice; @lime; @lubridate; @Matrix; @methods; @mice; @MSstats; @naniar; @psych; @purrr; @RColorBrewer; @ReactomePA; @readxl; @remotes; @rlang; @stats; @STRINGdb; @stringr; @survival1; @survival2; @tidyverse; @utils; @zoo]
...
Detected proteins were reported with a Protein Threshold [Unused ProtScore (confidence)] >0.05 and included in the quantitative analysis if identified with two or more peptides with >95% confidence.
False Discovery Rate (FDR) \nomenclature{FDR}{False Discovery Rate}analysis was also performed using the ProteinPilot.
ProteinPilot calculated P-values for the iTRAQ ratios and those with P > 0.05 were considered statistically significant.
Proteins with iTRAQ ratios $\ge\pm1.2$ \nomenclature{FC}{Fold change}fold change (FC) were used for network analysis.

### OpenMS analysis - iTRAQ {#openms-chap3}

The TripleTOF 5600 tandem mass spectrometer output files are in the ABSciex proprietary `.wiff` file format.
For analysis with OpenMS (version 2.6.0) conversion to an open file format, `.mzML` in this case, was needed and the docker image of ProteoWizard version 3.0.20287 was used for conversion, and peak picking was applied on conversion.[@chambers_cross-platform_2012]

OpenMS version 2.6.0 was used for further analysis.[@rost_openms_2016]
...
### OpenMS analysis - label-free {#openms-label-free}
...
As in section \@ref(openms-chap3), `PeptideIndexer` and `PSMFeatureExtractor` were used for annotation.
...
### Validation of iTRAQ results using enzyme-linked immunosorbent assay
...
Statistical analysis was perfomed using the statistical programming language `r R.version$version.string`.
Pairwise t tests with bonferroni adjusted P-values with the R `rstatix` package were used to assess differential abundance.

\clearpage

(ref:elisa-quant-duo-compare) Flow diagram illustrating ELISA process.

```{r, label='elisa-quant-duo-compare', fig.cap='(ref:elisa-quant-duo-compare)', out.width="100%"}
mermaid("
graph TD
    A(<b><center>Coating:</center></b><br>Antigen or antibody is absorbed on well in ELISA plate with coating buffer) -- remove liquid and wash plate --> B(<b><center>Blocking:</center></b><br>Buffer containing unrelated protein is used to block free sites in the wells)
    B -- remove liquid and wash plate --> C(<b><center>Detection:</center></b><br>Enzyme-conjugated antigen or antibody binds target antigen or antibody)
    C -- remove liquid and wash plate --> D(<b><center>Readout:</center></b><br>Substrate is catalysed by enzyme to generate coloured readout)
    E(Quatikine kits are optimized and validated for<br> specific sample types, whereas Duosets are<br> less costly and more flexible)
")
```
...
cnetplot <- readRDS("data/openms/reactomepa_cnetplots_quantification_output_2021-01-18.rds")
...
A total of `r nrow(fc_openms_wide)` and `r nrow(pp_fc_filtered)` unique, largely overlapping, proteins were identified across both runs for OpenMS and ProteinPilot respectively, though OpenMS consistently produced more proteins for each group (Tables \@ref(tab:openms-fc-table), \@ref(tab:proteinpilot-fc-table) and Figure \@ref(fig:openms-pp-venn)).
...
```{r, label='openms-pp-venn', fig.cap='(ref:openms-pp-venn)', fig.align = "center", out.width="60%"}
...
```{r, label='openms-pp-updown', fig.cap='(ref:openms-pp-updown)'}
## rbind data
...
```{r, label='openms-hmap-acute-c-1', fig.cap='(ref:openms-hmap-acute-c-1)', fig.align="center", fig.width=12, fig.height=8}
...
```{r, label='openms-hmap-subacute-c-1', fig.cap='(ref:openms-hmap-subacute-c-1)', fig.align="center", fig.width=13, fig.height=10}
...
```{r, label='openms-cnetp-acute-c-1', fig.cap='(ref:openms-cnetp-acute-c-1)', fig.align="center", fig.width=12, fig.height=10}
cnetplot$acute_c_improvers_vs_nonimprovers_run1
```
...
```{r, label='openms-cnetp-subacute-c-1', fig.cap='(ref:openms-cnetp-subacute-c-1)', fig.align="center", fig.width=12, fig.height=10}
cnetplot$subacute_c_improvers_vs_nonimprovers_run1
```
...
(ref:elisa-sig-plots1) Normalised estimated concentration of $\alpha$-2-macroglobulin. Estimates were calculated from the optical density of a standard curve produced via a DuoSet&reg ELISA. Plasma from each patient that made up the pooled iTRAQ samples was assayed and pairwise t-tests with bonferroni adjusted P-values were preformed to assess differential abundance.

```{r, label='elisa-sig-plots1', fig.cap='(ref:elisa-sig-plots1)'}
elisa_sig_plots <- readRDS("data/elisas/sig_plots_2021-03-16.rds")
elisa_sig_plots[[1]]
```

(ref:elisa-sig-plots2) Normalised estimated concentration of serum amyloid A1. Estimates were calculated from the optical density of a standard curve produced via a DuoSet&reg ELISA. Plasma from each patient that made up the pooled iTRAQ samples was assayed and pairwise t-tests with bonferroni adjusted P-values were preformed to assess differential abundance.

```{r, label='elisa-sig-plots2', fig.cap='(ref:elisa-sig-plots2)'}
elisa_sig_plots[[2]]
```

(ref:elisa-sig-plots3) Normalised estimated concentration of apolipoprotein A1. Estimates were calculated from the optical density of a standard curve produced via a Quantikine&reg ELISA. Plasma from each patient that made up the pooled iTRAQ samples was assayed and pairwise t-tests with bonferroni adjusted P-values were preformed to assess differential abundance.

```{r, label='elisa-sig-plots3', fig.cap='(ref:elisa-sig-plots3)'}
elisa_sig_plots[[3]]
```

(ref:elisa-sig-plots4) Normalised estimated concentration of retinol binding protein 4. Estimates were calculated from the optical density of a standard curve produced via a DuoSet&reg ELISA. Plasma from each patient that made up the pooled iTRAQ samples was assayed and pairwise t-tests with bonferroni adjusted P-values were preformed to assess differential abundance.

```{r, label='elisa-sig-plots4', fig.cap='(ref:elisa-sig-plots4)'}
elisa_sig_plots[[4]]
```
...
## Label-free results {#chap-4-results}
...
The mean number of down-regulated and up-regulated significant proteins in each group is `r mean(fc_summary_df$down_reg)`, and `r mean(fc_summary_df$up_reg)`.
...
<!--
***NOTE: maybe easier to just have a table showing the numbers?***
```{r, label='up-down-reg-table-chap4', eval=FALSE}
knitr::kable(fc_summary_df)
```
-->
...
## get volcano plot
knitr::include_graphics("figures/openms_protein_quantification/label_free/openms_volcano_plot_2021-08-10_0008.pdf")
```
...
knitr::include_graphics("figures/openms_protein_quantification/label_free/openms_volcano_plot_2021-08-10_0026.pdf")
```
...
```{r, label='openms-chap4-hmap-acute-c-1', fig.cap='(ref:openms-chap4-hmap-acute-c-1)', fig.align="center", fig.width=12, fig.height=8}
## read in heatmaps
...
```{r, label='openms-chap4-hmap-subacute-c-1', fig.cap='(ref:openms-chap4-hmap-subacute-c-1)', fig.align="center", fig.width=12, fig.height=8}
...
```{r, label='openms-chap4-cnetp-acute-c-1', fig.cap='(ref:openms-chap4-cnetp-acute-c-1)', fig.align="center", fig.width=12, fig.height=10}
## load data
...
```{r, label='openms-chap4-cnetp-subacute-c-1', fig.cap='(ref:openms-chap4-cnetp-subacute-c-1)', fig.align="center", fig.width=12, fig.height=10}
...
As with the iTRAQ data (section \@ref(kegg-chap3)), pathway analysis via the `pathview` R package returned the complement and coagulation cascade to be on the sole significant KEGG pathway.
...
A total of `r length(label_free_proteins)` and `r nrow(fc_openms_wide)` unique proteins were identified across the label-free and iTRAQ experiments respectively, with modest overlap in those identified (Figure \@ref(fig:itraq-label-free-venn)).
...
Another study of \nomenclature{ALS}{Amyotrophic lateral sclerosis}amyotrophic lateral sclerosis (ALS), a neurodegenerative disease, comparing gene expression between post-mortem spinal cord samples of ALS and controls also observed up-regulation of RBP1 in ALS spinal cord.[@malaspina_differential_2001]
...
The bodies first response to injury or infections, including SCI, is often referred to as the \nomenclature{APR}{Acute Phase Response}"acute phase response" (APR), which is non-specific, innate reaction that precedes more specific and situational immune reactions.[@gordon_acute-phase_1985; @gruys_acute_2005]
...
This provokes the liver to enter the APR and produce \nomenclature{APP}{Acute Phase Proteins}acute phase proteins (APPs) thus stimulating a greater immune response.[@anthony_systemic_2014; @fleming_remote_2012]
The hepatocytes that make up the majority of the liver biomass, express receptors that bind the aforementioned inflammatory cytokines; similarly the hepatic macrophage Kupffer cells also bind these cytokines, complement proteins and \nomenclature{LPS}{Lipopolysaccharide}lipopolysaccharide (LPS) and swiftly remove microorganisms, endotoxins and other debris from the blood.[@yang_clec4f_2013; @szalai_complement-dependent_2000; @crispe_hepatocytes_2016; @campbell_central_2005]
...
SCI studies in rodent and canine models have found the APPs \nomenclature{SA}{Serum Amyloid}serum amyloid (SA) A, SAP, CRP, fibrinogen, haptoglobin and a1-antichymotrypsin are elevated 4-24 hours post-injury in blood.[@pepys_acute_1983; @epstein_acute-phase_1999; @hall_docosahexaenoic_2012; @steel_major_1994]
...
The binding of LPS to Kupffer cells results in the production of a range of growth factors, including TNF-$\alpha$, multiple interleukins and \nomenclature{ROS}{Reactive Oxygen Species}reactive oxygen species (ROS), stimulating bone-marrow-derived monocytes and neutrophils to infiltrate the liver.[@myers_following_2019; @milosevic_gut-liver_2019; @kazankov_role_2019]
...
Two such aldehydic products of lipid peroxidation are \nomenclature{4-HNE}{4-hydroxynonenal}4-hydroxynonenal (4-HNE) or 2-propenal (acrolein), both of which have to characterised in experimental models of both brain and spinal injury.[@bains_antioxidant_2012; @hamann_acrolein_2009]
...
\nomenclature{PRX-2}{Peroxiredoxin 2}Peroxiredoxin 2 (PRX-2) is highly abundant in RBCs and intracellularly serves as an important anti-oxidant role in various cells types, including neurons.[@low_peroxiredoxin_2008]
...
Over-expression of PRX-2 is speculated to protect again ischaemic neuronal injury by modulating the redox-sensitive \nomenclature{ASK}{Apoptosis signal-regulating kinase}thioredoxin-apoptosis signal-regulating kinase (ASK) 1 signalling complex.[@gan_transgenic_2012]
...
The resulting inflammatory response occurs in parallel to the mechanical destruction of the \nomenclature{BSCB}{Blood-spinal cord barrier}blood-spinal cord barrier, and the development of tissue oedema and ischaemia combine to propagate damage to parts of the cord spared by the initial trauma.[@maikos_immediate_2007; @ahuja_traumatic_2017]
...
Surviving microglia change in shape and migration patterns, and begin to produce ROS, oxidative metabolites an pro-inflammatory cytokines.[@pineau_proinflammatory_2007; @bastien_cytokine_2014]
These cells can associate with damaged axons rapidly post-injury, but are thought to not actively phagocytose these cells until approximately 4 days post-trauma.[@bellver-landete_microglia_2019; @pineau_proinflammatory_2007; @greenhalgh_differences_2014]
...
Moving forward in the SCI pathology, newly proliferated and recruited microglia begin actively phagocytosing necrotic cell debris, and begin accumulating around the lesion epicentre.[@greenhalgh_differences_2014; @bellver-landete_microglia_2019; @pineau_proinflammatory_2007]
...
Originally developed as an antibody replacement therapy for immunodeficiency disorders, IVIG is a product comprised primarily of \nomenclature{IgG}{Immunoglobulin G}immunoglobulin G (IgG) taken from the blood plasma of healthy donors.[@bayry_intravenous_2011; @schwab_intravenous_2013]
IVIG therapy was found to increase platelet number in \nomenclature{ITP}{Idiopathic thrombocytopenic purpura}idiopathic thrombocytopenic purpura (ITP) patients, which lead to an interest in using it as an immunomodulatory therapy.[@imbach_high-dose_1981]
